...
client.on("messageCreate", (message) => {
  ...
  if (message.channel.id === config.groupId) {
    ...
    const yablength = message.content.split("").length;
    const xp = message.content.length

    console.log(db.get(message.author.id).characters)
    db.math("totalmessages", "+", 1);
    db.math(message.author.id, "+", 1, "messages")
    db.math(message.author.id, "+", yablength, "characters");
    db.math(message.author.id, "+", xp, "xp");
    console.log(db.get(message.author.id))
    db.set(message.author.id, convertToLevels(xp), "level")
    if (db.get(message.author.id).level > db.get(message.author.id).previouslevel) {
      var user = message.author.id
      message.channel.send(`<@${user}> has reached level ${userlevel}!`);
      }
    db.set(message.author.id, db.get(message.author.id).level, "previouslevel")
      ...
      .then((data1) => {
        ...
        fetch(`https://united-api.mixerno.space/youtube/channel?id=${cid}`)
          ...
          .then((data3) => {
            ...
            if (Array.isArray(data3.data) && data3.data.length > 0) {
              ...
              if (lastUpdate) {
                ...
                secon1d = secon1d.trim(); // Remove any leading or trailing whitespace
                if (/^[1-9]$/.test(secon1d)) {
                  secon1d = "0" + secon1d; // Add a "0" before the single digit
              } else {
                  secon1d = Math.floor(Number(secon1d)); // Convert to number and floor it
              }
          ...
          var estya1b = "Estimated"
            if (cid = message.content.toLowerCase().includes("UC-lHJZR3Gqxm24_Vd_AJ5Yw"))
            {
...
${provided}`);
  ...
  if (message.channelId === config.groupId && message.content.toLocaleLowerCase().startsWith("ify!info")) {
    ...
    message.reply(`You have ${userxp} XP and are on level ${userlevel}! You also do have ${usermessages} messages and ${usercharacters} characters sent.`)
  }
